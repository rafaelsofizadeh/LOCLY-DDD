...
[
  ...
  axios
    .get(
      `https://appservices.postdanmark.dk/PriceService.svc/GetPrices/180/${w}`,
    )
    .then(({ data }) => (console.log(data), uk.push(data))),
);

setTimeout(
  () =>
    writeFile(
      join(__dirname, './UK.json'),
      JSON.stringify(uk, null, 2),
      err => {
        if (err) throw err;
